...
#define SHUFFLE2(V, X,Y) float3(_mm_shuffle_ps((V).m, (V).m, _MM_SHUFFLE(Y,Y,Y,X)))
...
{   
	...
	VM_INLINE explicit float4(const float *p){ m = _mm_set_ps(p[2], p[2], p[1], p[0]); }
	VM_INLINE explicit float4(float x, float y, float z) { m = _mm_set_ps(z, z, y, x); }
	VM_INLINE explicit float4(__m128 v) { m = v; }
	VM_INLINE float4 float4i(int x, int y, int z) { return float4((float)x, (float)y, (float)z); }
...
TODO:
- float2
- float4 member functions 
- operators for float2 and float4
*/